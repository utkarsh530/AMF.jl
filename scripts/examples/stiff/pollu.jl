...
M = MatrixOperator(zeros(20, 20); update_func! = fjac)
...
prob = ODEProblem(func, u0, (0.0, 60.0))
...
sol.destats
oprob = ODEProblem(ODEFunction(f; jac = fjac), u0, (0.0, 60.0))
...
@benchmark sol = solve(prob, ROS34PW1a(; solver_options...), dt = 3e-4, adaptive = false)
# BenchmarkTools.Trial: 1 sample with 1 evaluation.
#  Single result which took 8.732 s (8.80% GC) to evaluate,
#  with a memory estimate of 1.73 GiB, over 33200158 allocations.
sol.destats

@benchmark osol = solve(oprob, ROS34PW1a(), dt = 3e-4, adaptive = false)
# BenchmarkTools.Trial: 1 sample with 1 evaluation.
#  Single result which took 15.839 s (0.00% GC) to evaluate,
#  with a memory estimate of 175.49 MiB, over 1600102 allocations.
osol.destats
