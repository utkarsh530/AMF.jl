...
using ForwardDiff
du = fill!(Array{Num}(undef, N), 0)
@variables u[1:109]
sbml_model!(du, u0, prob.p, prob.tspan)
J = fill!(Array{Num}(undef, N, N), 0)
J = ForwardDiff.jacobian!(J, (du, u) -> prob.f, du, u0)

J2 = UpperTriangular(J - Diagonal(J))
out, fjac_upper_expr = build_function(J2, u; skipzeros = true)
fjac_upper_1 = eval(fjac_upper_expr)
fjac_upper_sym(J, u, p, t) = fjac_upper_1(J, u)
J2 = LowerTriangular(J)
out, fjac_lower_expr = build_function(J2, u)
fjac_lower_1 = eval(fjac_lower_expr)
fjac_lower_sym(J, u, p, t) = fjac_lower_1(J, u)
M = MatrixOperator(
    zeros(20, 20);
    update_func! = ForwardDiff.jacobian!(J, (du, u) -> prob.f, du, u0),
)
I_N = Diagonal(ones(N))
# J_op = MatrixOperator(zeros(20, 20); update_func! = fjac)
# J_op = cache_operator(J_op, zeros(N^2))
J1_op = MatrixOperator(
    UpperTriangular(zeros(N, N));
    update_func! = UpperTriangular(ForwardDiff.jacobian!(J, (du, u) -> prob.f, du, u0)),
)
J2_op = MatrixOperator(
    LowerTriangular(zeros(N, N));
    update_func! = LowerTriangular(ForwardDiff.jacobian!(J, (du, u) -> prob.f, du, u0)),
)
J_op = J1_op + J2_op
J_op = cache_operator(J_op, zeros(N^2))
W1_op = MatrixOperator(
    ...
    update_func! = (M, u, p, t; dtgamma) ->
        (fjac_upper_sym(M, u, p, t); @. M = I_N - dtgamma * M),
    accepted_kwargs = (:dtgamma,),
)
W2_op = MatrixOperator(
    LowerTriangular(zeros(N, N));
    update_func! = (M, u, p, t; dtgamma) ->
        (fjac_lower_sym(M, u, p, t); @. M = I_N - dtgamma * M),
    accepted_kwargs = (:dtgamma,),
)
...
func = ODEFunction(prob.f; jac_prototype = J_op, W_prototype)
prob = ODEProblem(func, u0, (0.0, 60.0)) #LU problem
tstep = 1e-3
@time sol = solve(prob, KenCarp4(; solver_options...), dt = tstep);
#sol.destats #LU solve with KenCarp
#@time sol = solve(prob, ROS34PW1a(;solver_options...), dt = tstep);
#sol.destats #LU solve with ROS34PW1a
#oprob = ODEProblem(ODEFunction(f;jac = ForwardDiff.jacobian!(J, (du,u) -> prob.f, du, u0)), u0, (0.0,60.0)) #concrete problem
#@time osol = solve(oprob, KenCarp4(), dt = tstep);
#osol.destats #Concrete solve with KenCarp4
