...
function solve2d(A, B, C, N, g, init_func, alg::FiniteDifferenceMethod; 
    ...
    elseif alg.strategy in ("exact_W", "amf_W")
        ...
        elseif alg.strategy == "amf_W"
            ...
            I_N = Diagonal(ones(N))
            _W1_op = MatrixOperator(I_N - A * D; update_func! = (M, u, p, t; dtgamma) -> (@. M = I_N - dtgamma * A * D), accepted_kwargs=(:dtgamma,)) 
            _W2_op = MatrixOperator(I_N - B * D; update_func! = (M, u, p, t; dtgamma) -> (@. M = I_N - dtgamma * B * D), accepted_kwargs=(:dtgamma,)) 
